...
export default {
  ...
  computed: {
    ...
    routes() {
      ...
      var hasAdd2=JSON.parse(sessionStorage.getItem('routesInfo'));
      if(!hasAdd){
        var dR = new Array(),role=this.$store.state.user.routerRole;
        var role=sessionStorage.getItem('vue_admin_info') != 'null'?JSON.parse(sessionStorage.getItem('vue_admin_info')).routerRole:0;
